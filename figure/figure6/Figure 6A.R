...
cellinfo <- single_data@meta.data[,c('wsnn_res_0.5_cell_type',"nFeature_RNA","nCount_RNA")]
colnames(cellinfo)=c('celltype','nGene' ,'nUMI')
cellTypes <-  as.data.frame(subset(cellinfo,select = 'celltype'))
selectedResolution <- "celltype"

##import loom data from fetal_pyscenic.sh
loom <- open_loom("SingleCell.out.auc_mtx.loom") 
regulons_incidMat <- get_regulons(loom, column.attr.name="Regulons")

regulons <- regulonsToGeneLists(regulons_incidMat)
saveRDS(regulons,file = "fetal_regulonsToGeneLists.rds")
...
##import some important TF
library(openxlsx)
TF<-read.xlsx("fetal TF list.xlsx")
...
regulonActivity_byGroup_Scaled<-regulonActivity_byGroup_Scaled[,c(3:11,1:2)]
regulonActivity_byGroup_Scaled<-regulonActivity_byGroup_Scaled[c(5:23,1:4),]
...
p<-pheatmap(t(regulonActivity_byGroup_Scaled),cluster_cols = F,cluster_rows = F,
            ...
            border_color="black")
ggsave(filename = "fetal_regulonActivity1.pdf",p)


##this code is to change the y label to the left in pheatmap
